...
class MockProvider(Provider):
    def complete(self, model: ModelTarget, prompt: PromptCase) -> Completion:
        ...
        if prompt.id in mock_responses:
            response = mock_responses[prompt.id]
        elif model.mock_response is not None:
            response = model.mock_response
        else:
            response = "Mock response generated by pangolin-eval."
        return Completion(
            text=response,
